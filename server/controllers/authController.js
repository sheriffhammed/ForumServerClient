...
const handleLogin = async (req, res) => {
    ...
    if(!retreiveUser) return res.sendStatus(401).json({message : "Unauthorised User"})
    ...
    if(passwordMatch){
       // create JWTs
        const {id, firstName, lastName,email,phone}  = retreiveUser
        const jwtUserDetails = { id, firstName, lastName, email,phone }
        ...
        if(addRefreshToken[0] === 0)            
            return res.sendStatus(400).json({message: `Refresh Token Could not be added to database`})
        ...
        res.json({ accessToken,userId: id, firstName, lastName,email,phone });
    } else {
